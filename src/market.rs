...
pub struct Market {
    pub client: Client,
    pub recv_window: u64, // This field seems unused in this impl block but kept for struct definition consistency
}
...
impl Market {
        ...
        S: Into<String>,
    {
        ...
        self.client.get(API::Spot(Spot::Depth), Some(request)).await // .await added
    }
        ...
        S: Into<String>,
    {
        ...
        parameters.insert("symbol".into(), symbol.into());
        let request = build_request(parameters);
        self.client.get(API::Spot(Spot::Price), Some(request)).await // .await added
    }
