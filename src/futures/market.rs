...
impl FuturesMarket {
        ...
        S: Into<String>,
    {
        ...
        self.client.get(API::Futures(Futures::Depth), Some(request)).await // .await added
    }
    ...
    pub async fn get_trades<S>(&self, symbol: S) -> Result<Trades> // async added
    where
        S: Into<String>,
    {
        let mut parameters: BTreeMap<String, String> = BTreeMap::new();
    ...
    pub async fn get_historical_trades<S1, S2, S3>( // async added
    ...
    pub async fn get_all_24h_price_stats(&self) -> Result<Vec<PriceStats>> { // async added
        self.client.get(API::Futures(Futures::Ticker24hr), None).await // .await added
    }
    ...
    pub async fn get_all_prices(&self) -> Result<crate::model::Prices> { // async added
        self.client.get(API::Futures(Futures::TickerPrice), None).await // .await added
    }

    // Symbols order book ticker
    // -> Best price/qty on the order book for ALL symbols.
    pub async fn get_all_book_tickers(&self) -> Result<BookTickers> { // async added
        self.client.get(API::Futures(Futures::BookTicker), None).await // .await added
    }
    ...
    pub async fn get_mark_prices(&self) -> Result<MarkPrices> { // async added
        self.client.get(API::Futures(Futures::PremiumIndex), None).await // .await added
    }

    pub async fn get_all_liquidation_orders(&self) -> Result<LiquidationOrders> { // async added
        self.client.get(API::Futures(Futures::AllForceOrders), None).await // .await added
    }

    pub async fn open_interest<S>(&self, symbol: S) -> Result<OpenInterest> // async added
    where
        S: Into<String>,
    {
        let mut parameters: BTreeMap<String, String> = BTreeMap::new();
    ...
    pub async fn open_interest_statistics<S1, S2, S3, S4, S5>( // async added
